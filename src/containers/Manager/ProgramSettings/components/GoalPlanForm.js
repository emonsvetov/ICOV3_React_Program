...
const GoalPlanForm = ({ program, goalplan, setOpen }) => {
  ...
  const [goalExceededProgramCallbacks, setGoalExceededProgramCallbacks] =
    useState([]);
  ...
  useEffect(() => {
    ...
    getEvents(program.organization_id, program.id).then((items) => {
      setEvents(labelizeNamedData(items));
    });
    getGoalExceededProgramCallbacks(program.organization_id, program.id).then(
      (items) => {
        setGoalExceededProgramCallbacks(labelizeNamedData(items));
      }
    );
    getGoalMetProgramCallbacks(program.organization_id, program.id).then(
      (items) => {
  ...
  const onSubmitEdit = (values) => {
    ...
    goalPlanData.automatic_frequency = automatic_frequency
      ...
      : null;
    goalPlanData.automatic_value = automatic_value ? automatic_value : null;
