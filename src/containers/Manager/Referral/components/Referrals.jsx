...
const Referrals = ({program, organization}) => {
    ...
    const RenderActions = ({row}) => {
      return (
          <span>
              <Link to={{}} onClick={() => onClickEditReferral(row.original.id)}><PencilIcon style={{marginRight: "0.5rem"}}/>Edit</Link> 
              <span style={{width:'2.5rem', display: 'inline-block'}}></span>
              <Link to={{}} className='delete-column' onClick={(e) => {if(window.confirm('Are you sure to delete this Administrator?')){onDeleteReferral(e, row.original.id)}}}><TrashIcon style={{marginRight: "0.5rem"}}/>Delete</Link>
          </span>
      )
    }
    ...
    return (
      ...
      <ModalWrapper  name={modalName} isOpen={isOpen} setOpen={setOpen} toggle={toggle} referral={referral} setReferral={setReferral}/>
      </>
    )
  }
